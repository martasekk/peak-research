...
5. **Mandatory Subagent Dispatch**: When executed by an agent, retrieval passes (discovery, targeted, contradiction, gap) must be dispatched in parallel via native subagents (`invoke_subagent` with `peak-retriever` or `research` role, max 3 concurrent) adhering strictly to the leaf-only retrieval contract.
