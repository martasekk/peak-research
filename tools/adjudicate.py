...
whichever key is set. Example (NVIDIA NIM):
    ...
    export PEAK_ADJUDICATION_PROVIDER=openai
    export PEAK_ADJUDICATION_BASE_URL=https://integrate.api.nvidia.com/v1
...
_DEFAULT_MODELS = {
    ...
    "openai": "nvidia/llama-3.1-nemotron-ultra-253b-v1",
}
...
def model() -> str:
    return _env("PEAK_ADJUDICATION_MODEL") or _DEFAULT_MODELS[provider()]
...
def _call_openai(system: str, user: str, schema: dict, effort: str) -> str:
...
def _call(system: str, user: str, schema: dict, effort: str = "medium",
          force: bool = False, required: tuple[str, ...] = ()) -> dict:
    """One structured-output request, disk-cached by provider + model + prompt."""
    prov = provider()
    key = json.dumps({"p": prov, "m": model(), "s": system, "u": user,
    ...
    if prov == "openai":
        text = _call_openai(system, user, schema, effort)
    ...
    try:
        ...
        raise AdjudicationUnavailable("model %r not available: %s" % (model(), e)) from e
...
def selftest() -> int:
    ...
    ok, reason = available()
    print("provider : %s" % (provider() if ok else "n/a"))
    print("model    : %s" % model())
    if provider() == "openai":
