...
## 0. CURATED CATALOG PATH (FIXED)
The catalog is NOT in Downloads. Correct path (verified):
- `D:\peak-search\curated_research_source_catalog.md`   (v3.0, 110 sources)
- Backup copy: `C:\Users\Martin\Downloads\peak-search-backup\curated_research_source_catalog.md`
Rule #6 still holds: never cite aggregator/abstract when the original is available.
...
python tools/retrieval.py verify records.json               # S19 gate: every cited source resolves
```
...
- Parent merges children's JSON into one evidence table and runs `verify_records` (S19).
