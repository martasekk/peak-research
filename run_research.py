#!/usr/bin/env python3
"""
peak-research-v2/run_research.py — FULLY IMPROVED

All upgrades:
1. LLM-assisted claim typing (via local heuristic + OpenRouter if available)
2. Deduplicated contradiction detection with directional analysis
3. Relevance filtering (keyword overlap + citation threshold)
4. Full-text fetch via Jina for top-N papers
5. Multi-strategy search expansion (citation chaining, author search, venue search)
6. Subagent parallel retrieval (max 3)
6. Complete G1-G5 gates with final .md artifact
"""
...
if __name__ == "__main__":
    main()
'''
    SUBAGENT_SCRIPT.write_text(script, encoding="utf-8")
...
def phase2_retrieve(plan: dict, rundir: Path) -> dict:
    ...
    # Always regenerate: this file is generated code, and writing it only when absent
    # meant edits to create_subagent_script() never reached the script actually run.
    create_subagent_script()
...
def get_pdf_url_via_core(doi: str, title: str) -> str | None:
    ...
    try:
        ...
        req = urllib.request.Request(url, headers={"User-Agent": "peak-research-v2"})
        with urllib.request.urlopen(req, timeout=15) as r:
            data = json.loads(r.read().decode("utf-8", "ignore"))
...
# ============================================================
def main():
    ap = argparse.ArgumentParser(description="peak-research-v2 FULLY IMPROVED orchestrator")
    ...
    ap.add_argument("--output-dir", default=None,
                    help="where the .md deliverable goes (default: CONFIG output_dir)")
    ...
    rundir = HERE / "runs" / slugify(args.topic)
    rundir.mkdir(parents=True, exist_ok=True)
    print(f"== peak-research-v2 IMPROVED run: {args.topic}")
